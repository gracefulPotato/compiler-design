...
structdef : TOK_STRUCT TOK_IDENT '{' '}' {destroy($3,$4);$$=$1->adopt($2);}
          | TOK_STRUCT TOK_IDENT '{' fielddecl ';' '}' {destroy($3,$5);destroy($6);$$=$1->adopt($2,$4);}
          | TOK_STRUCT TOK_IDENT '{' fielddecl';'fielddecl '}'{destroy($3,$5);destroy($7);$$=$1->adopt($2,$4);$$->adopt($6);}
          ;
...
expr : expr BINOP expr              { $$ = $2->adopt ($1, $3); }
     ...
     | constant                     { $$ = $1; }
     ;
allocator : TOK_NEW TOK_IDENT '('')' {destroy($3,$4);$$=$1->adopt($2);}
